...
client.on("ready", async () => {
  ...
  try {
    await setupAgent();
    console.log("🎉 Bot fully operational! Ready for groups and private chats!");
...
const setupAgent = async () => {
  try {
    ...
    console.log("🔧 Setting up OpenAI model...");
    ...
    executor = await initializeAgentExecutorWithOptions(
      ...
      model,
      {
        ...
        agentKwargs: {
          systemMessage: "Kamu adalah asisten AI yang selalu menjawab dalam bahasa Indonesia. Berikan jawaban yang informatif, ramah, dan mudah dipahami. Jika ada pertanyaan dalam bahasa asing, tetap jawab dalam bahasa Indonesia."
        }
      }
    );
...
client.on("message", async (msg) => {
  try {
    ...
    if (!isReady) {
      ...
      await msg.reply("🤖 Bot sedang loading, tunggu sebentar ya...");
      return;
    }
    ...
    if (isGroupMsg && isAIActive) {
      if (body.startsWith("/")) {
        ...
        console.log(`🤖 AI Mode: Processing message from ${authorName}: "${body}"`);
        aiModeResponseCount.set(from, currentCount + 1);

        try {
          if (!executor) {
            await msg.reply("⏳ AI sedang dipersiapkan, tunggu sebentar...");
            await setupAgent();
          }
          ...
          console.log(`✅ AI Mode response sent (${currentCount + 1}/${AI_MODE_MAX_RESPONSES})`);
      ...
      if (mode === "ai_mode") {
        ...
        if (!executor) {
          ...
          try {
            ...
            await msg.reply("❌ AI bermasalah saat ini. Coba lagi nanti.");
            return;
          }
        }

        try {
          console.log(`🤖 Private chat AI mode - processing: "${body}"`);
          const response = await Promise.race([
...
const processTrigger = async (msg, triggerType, cleanInput, isGroupMsg) => {
  ...
  if (!executor && (triggerType === 'ai' || triggerType === 'quiz' || triggerType === 'sports' || triggerType === 'news' || triggerType === 'weather' || triggerType === 'translate' || triggerType === 'crypto')) {
    console.log("⚠️ AI not ready, setting up...");
    await msg.reply("⏳ AI sedang dipersiapkan, tunggu sebentar...");
    try {
      await setupAgent();
  ...
  if (triggerType === 'ai') {
    ...
    try {
      console.log(`🤖 Processing AI chat: "${cleanInput}"`);
      const response = await Promise.race([
        executor.invoke({ 
...
Buat soal yang menarik dan tidak terlalu sulit.` 
  ...
  if (triggerType === 'sports') {
    try {
      console.log(`⚽ Processing sports request`);
      const response = await Promise.race([
        executor.invoke({ 
      ...
      console.log(`✅ Sports response sent`);
    } catch (error) {
      await handleAIError(msg, error);
    }
    return;
  ...
  if (triggerType === 'news') {
    try {
      ...
      console.log(`✅ News response sent`);
    } catch (error) {
      await handleAIError(msg, error);
    }
    return;
  ...
  if (triggerType === 'weather') {
    ...
    try {
      console.log(`🌤️ Processing weather request for: ${city}`);
      const response = await Promise.race([
        executor.invoke({ 
  ...
  if (triggerType === 'translate') {
    ...
    try {
      console.log(`🔄 Processing translate request: "${cleanInput}"`);
      const response = await Promise.race([
        executor.invoke({ 
      ...
      console.log(`✅ Translate response sent`);
    } catch (error) {
      await handleAIError(msg, error);
    }
    return;
  ...
  if (triggerType === 'crypto') {
    try {
      console.log(`💰 Processing crypto request`);
      const response = await Promise.race([
        executor.invoke({ 
      ...
      console.log(`✅ Crypto response sent`);
    } catch (error) {
      await handleAIError(msg, error);
    }
    return;
...
console.log("🔑 Make sure your .env file contains OPENAI_API_KEY and TAVILY_API_KEY");
...
console.log("🔧 Starting bot with AI Mode support...");
