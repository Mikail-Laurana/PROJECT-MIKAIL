...
class Player(pygame.sprite.Sprite):
    ...
    def draw_particle_trail(self, x, y, color=(255, 255, 255)):
        ...
        self.particles.append(
                [[x - 2, y - 8], [random.randint(0, 25) / 10 - 1, random.choice([0, 0])],
                 random.randint(5, 8)])
...
def death_screen():
    ...
    if attempts == 5:
        game_over_sound = pygame.mixer.Sound("Pydash-main/music/gameOver.mp3")
...
def reset():
    ...
    # if level == 1:
    #     pygame.mixer.music.load(os.path.join("Pydash-main\music", "bg.mp3"))
...
# [Kode sebelumnya tetap sama sampai fungsi draw_stats]
...
def draw_stats(surf, money=0):
    ...
    # Batasi nilai fill antara 0 dan BAR_LENGTH
    fill = min(max(fill, 0), BAR_LENGTH)
    
    # Hitung indeks warna dengan aman
    color_index = min(int(fill / (BAR_LENGTH / len(progress_colors))), len(progress_colors) - 1)
    col = progress_colors[color_index]
    
    for i in range(1, min(money, 3)):  # Batasi maksimal 6 coin
        screen.blit(coin, (BAR_LENGTH + (i-1)*35, 25))
    
    fill += 0.5  # Tetap increment fill tapi sekarang dibatasi
...
# [Kode sebelumnya tetap sama sampai fungsi won_screen]

def won_screen():
    """show this screen when beating a level with animation"""
    global attempts, level, fill, coins

    # Animasi kemenangan
    animation_frames = []
    for i in range(1, 301):  # Load 300 frame animasi
        frame = pygame.image.load(f"images/frame/{i:05}.png")
    ...
    for frame in animation_frames:
        ...
        pygame.display.flip()
        pygame.time.delay(20)  # 20ms per frame

    # Setelah animasi selesai, tampilkan frame terakhir sebagai background
    screen.blit(animation_frames[-1], (0, 0))

    # Tampilkan teks kemenangan di atas frame terakhir
    attempts = 0
    player_sprite.clear(player.image, screen)

    txt_win1 = txt_win2 = "Nothing"
    if level == 1 and coins == 3:
        txt_win1 = f"Coin {coins}/3!"
        txt_win2 = "You beat the game, Congratulations"
    else:
        txt_win1 = f"Level {level}"
        txt_win2 = f"Coins: {coins}/3."

    # Delay untuk tampilan teks
    delay_time = 500  # 500ms atau 0.5 detik per teks

    # Tampilkan teks pertama
    txt1_surface = font.render(txt_win1, True, BLUE)
    screen.blit(txt1_surface, (100, 500))
    pygame.display.flip()
    pygame.time.delay(delay_time)

    # Tampilkan teks kedua setelah delay
    txt2_surface = font.render(txt_win2, True, BLUE)
    screen.blit(txt2_surface, (100, 530))
    pygame.display.flip()
    pygame.time.delay(delay_time)

    # Gabungkan kedua teks untuk tampilan akhir
    txt_final = f"{txt_win1} {txt_win2}! Press SPACE to restart, or ESC to exit"
    final_surface = font.render(txt_final, True, BLUE)
    screen.blit(final_surface, (100, 560))
    pygame.display.flip()
...
avatar = pygame.image.load(r"images\avatar.png")
...
spike = pygame.image.load(r"images\obj-spike.png")
spike = resize(spike)
coin = pygame.image.load(r"images\coin.png")
coin = pygame.transform.smoothscale(coin, (32, 32))
block = pygame.image.load(r"images\block_1.png")
block = pygame.transform.smoothscale(block, (32, 32))
orb = pygame.image.load(r"images\orb-yellow.png")
orb = pygame.transform.smoothscale(orb, (32, 32))
trick = pygame.image.load(r"images\obj-breakable.png")
trick = pygame.transform.smoothscale(trick, (320, 320))
...
# music
# music = pygame.mixer_music.load(r"music\bossfight-Vextron.mp3")
music = pygame.mixer_music.load(r"music\bgmPixel.mp3")
pygame.mixer_music.play()
...
while not done:
    ...
    player.vel.x = 6
    if (player.rect.left > 800 or player.rect.right < 0 or player.rect.left > 800 or player.rect.top > 600 ):
        player.died = True

    # Perbaikan kondisi game over



    # [Kode render dan event handling tetap sama]
